...
public class DBMSBoundary extends GlobalData{
    ...
    public void inserisciLottiInSpedizione(int idSpedizione, LinkedList<LottoSpedizione> ordine) {
    ...
    public void modificaSpedizione(int IDSpedizione, LinkedList<LottoSpedizione> ordine ) {
    ...
    public LinkedList<Spedizione> getConsegne(String currentDate, int ID_FARMACIA) {
        ...
        try {
            Connection conn = DriverManager.getConnection(DB_URL);
            String sql = "SELECT IDSpedizione, dataConsegna ,Lotto.dataScadenza, Lotto.dataProduzione, Farmaco.nomeFarmaco, principioAttivo, FKLotto, quantita, Farmaco.IDFarmaco FROM Lotto_Spedizione, Lotto, Farmaco, Spedizione WHERE Spedizione.IDSpedizione = Lotto_Spedizione.FKSpedizione AND Lotto_Spedizione.FKLotto = Lotto.codiceLotto AND Lotto.FKFarmaco = Farmaco.IDFarmaco AND FKFarmacia =? AND dataConsegna<=? AND Spedizione.statoSpedizione=1 ORDER BY IDSpedizione";
            PreparedStatement preparedStatement = conn.prepareStatement(sql);
    ...
    public void inserisciSegnalazione(int IDSpedizione, int IDUtente, String segnalazione) {
    ...
    public void aggiornaInventarioFarmacia(int ID_FARMACIA, int IDSpedizione,LinkedList<LottoSpedizione> listaCarico) {
        ...
        try {
            ...
            for(LottoSpedizione ls : listaCarico){
                ...
                sql = "SELECT * FROM Farmaco WHERE IDFarmaco = ?";
                preparedStatement = conn.prepareStatement(sql);
                preparedStatement.setInt(1, ls.getIDFarmaco());
                resultSet = preparedStatement.executeQuery();
                ...
                if(!resultSet.next()){
                    ...
                    sql = "SELECT tipologia FROM Farmaco WHERE IDFarmaco = ?";
                    preparedStatement = conn.prepareStatement(sql);
                    preparedStatement.setInt(1, ls.getIDFarmaco());
                    resultSet = preparedStatement.executeQuery();

                    if(resultSet.next()) tipologia = resultSet.getString("tipologia");

                    conn.close();
                    conn = DriverManager.getConnection(DB_URL_F);
                    stat = conn.createStatement();
                    sql = "INSERT INTO Farmaco (IDFarmaco, nomeFarmaco, principioAttivo, tipologia) VALUES (?,?,?,?)";
                    preparedStatement = conn.prepareStatement(sql);
                    preparedStatement.setInt(1, ls.getIDFarmaco());
                    preparedStatement.setString(2, ls.getNomeFarmaco());
                    preparedStatement.setString(3, ls.getPrincipioAttivo());
                    ...
                    preparedStatement.setInt(5, ls.getIDFarmaco());
                    row = preparedStatement.executeUpdate();
                    ...
                    if(resultSet.next()){
                        sql = "UPDATE Lotto SET quantitaLotto = quantitaLotto + ? WHERE codiceLotto = ?";
                        preparedStatement = conn.prepareStatement(sql);
                        preparedStatement.setInt(1, ls.getQuantita());
                        preparedStatement.setString(2, ls.getCodiceLotto());
                        row = preparedStatement.executeUpdate();
    ...
    public void updateContratto(LinkedList<FarmacoContratto> farmaciContratto, int periodo, int IDContratto){
        ...
        try {
            ...
            for(FarmacoContratto fc : farmaciContratto){
                if(fc.getIDFarmaco() != 0 && fc.getQuantitaRichiesta()!=0) {
    ...
    */
    ...
    public LinkedList<Farmaco> getFarmaciNonDaBanco() {
        ...
        try {
            Connection conn = DriverManager.getConnection(DB_URL);
            Statement stat = conn.createStatement();
            String sql = "SELECT * FROM Farmaco LEFT JOIN Lotto ON Farmaco.IDFarmaco = Lotto.FKFarmaco WHERE tipologia NOT IN ('OTC') ORDER BY nomeFarmaco;";
            PreparedStatement preparedStatement = conn.prepareStatement(sql);
            ResultSet resultSet = preparedStatement.executeQuery();
            int IDFarmaco = -1;
    ...
    public LinkedList<Contratto> getContrattiFarmacia(){
        LinkedList<Contratto> listaContratti = new LinkedList<>();
        try {
            Connection conn = DriverManager.getConnection(DB_URL);
            Statement stat = conn.createStatement();
            String sql = "SELECT * FROM Contratto, Farmaco_Contratto WHERE Contratto.IDContratto = Farmaco_Contratto.FKContratto ORDER BY IDContratto";
            PreparedStatement preparedStatement = conn.prepareStatement(sql);
            ResultSet resultSet = preparedStatement.executeQuery();
            int IDContratto = -1;
            ...
            while(resultSet.next()) {
                ...
                if (resultSet.getInt("IDContratto") != IDContratto && IDContratto != -1) {
                    listaContratti.add(c);
                    c = new Contratto();
                }

                if (resultSet.getInt("IDContratto") != IDContratto) {

                    c.setIDContratto(resultSet.getInt("IDContratto"));
                    c.setPerioditicita(resultSet.getInt("perioditicita"));
                    c.setIDSede(resultSet.getInt("FKFarmacia"));
                    c.setUltimaConsegna(String.valueOf(resultSet.getDate("UltimaConsegna")));

                    IDContratto = resultSet.getInt("IDContratto");

                    fc = new FarmacoContratto();

                    fc.setIDFarmaco(resultSet.getInt("FKFarmaco"));
                    fc.setIDContratto(resultSet.getInt("FKContratto"));
                    fc.setQuantitaRichiesta(resultSet.getInt("quantitaRichiesta"));
                    c.addListaFarmaciContratto(fc);
    ...
    public int insertNewSpedizione(int IDSede){
